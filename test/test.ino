...
void setup(){
  ...
  setResolution();
  setColor();
  writeReg(0x11, 11); // Change this if cam is not clear (11 to 9 ~ 13)
...
void loop(){
  char readByte = 0;
  touch_status = digitalRead(pin_touch);

  if(Serial.available() > 0){
    readByte = Serial.read();
    if(readByte == 'O' || readByte == 'o')
      open_door();
     else if(readByte == 'C' || readByte == 'c')
      close_door();
  }
